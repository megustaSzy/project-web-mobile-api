...
export default function ChooseUs() {
  ...
  return (
    <section className="relative bg-gray-100 bg-[url('/images/peta.svg')] bg-gradient-to-b from-[#f8fafc] to-white py-20 overflow-hidden bg-no-repeat bg-center bg-cover">
      
      <div className="absolute inset-0 bg-gradient-to-b from-[#f2f6f9] to-white/90 z-0"></div>

      <div className="absolute inset-0 flex justify-center items-center pointer-events-none opacity-20 z-0">
        <div className="w-[90%] h-[90%] bg-[radial-gradient(circle_at_center,rgba(59,130,246,0.1)_1px,transparent_1px)] [background-size:20px_20px]" />
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="flex justify-center">
            <img src="/images/5.png" alt="Liburan" className="max-w-md w-full h-auto drop-shadow-xl" />
          </div>
          ...
          <div className="space-y-4">
            {items.map((item, i) => {
              ...
              return (
                <div key={i} className="flex items-center bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
                  <img src={item.img} alt={title.trim()} className="w-12 h-12 mr-4" />

                  <div>
                    <h3 className="text-xl font-bold text-gray-900">{item.value}</h3>
                    <p className="text-gray-800 text-sm leading-tight">
                      <span className="font-semibold block">{title.trim()}</span>
