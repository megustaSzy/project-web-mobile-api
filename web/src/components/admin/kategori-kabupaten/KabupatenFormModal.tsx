"use client";

import { KabupatenFormModalProps } from "@/types/admin/kategori-kabupaten";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Loader2, Upload } from "lucide-react";

export default function KabupatenFormModal({
  open,
  editId,
  nameInput,
  saving,
  onClose,
  onSave,
  onNameChange,
  onImageChange,
}: KabupatenFormModalProps) {
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] rounded-2xl">
        <DialogHeader>
          <DialogTitle className="text-xl">
            {editId ? "Edit Kabupaten" : "Tambah Kabupaten"}
          </DialogTitle>
          <DialogDescription>
            {editId
              ? "Perbarui informasi kabupaten yang sudah ada"
              : "Tambahkan kabupaten baru ke dalam sistem"}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="name" className="text-sm font-medium">
              Nama Kabupaten
            </Label>
            <Input
              id="name"
              value={nameInput}
              onChange={(e) => onNameChange(e.target.value)}
              placeholder="Contoh: Lampung Selatan"
              className="rounded-lg"
              disabled={saving}
            />
          </div>

          <div className="space-y-2">
            <Label htmlFor="image" className="text-sm font-medium">
              Foto Kabupaten
            </Label>
            <div className="relative">
              <Input
                id="image"
                type="file"
                onChange={(e) => onImageChange(e.target.files?.[0] ?? null)}
                className="rounded-lg file:mr-4 file:px-4 file:py-2 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer"
                accept="image/*"
                disabled={saving}
              />
              <Upload className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
            </div>
            <p className="text-xs text-muted-foreground">
              Format: JPG, PNG, atau WEBP. Maksimal 2MB
            </p>
          </div>
        </div>

        <DialogFooter className="gap-2 sm:gap-2">
          <Button
            type="button"
            variant="outline"
            onClick={onClose}
            disabled={saving}
            className="rounded-lg"
          >
            Batal
          </Button>
          <Button
            type="button"
            onClick={onSave}
            disabled={saving || !nameInput.trim()}
            className="bg-blue-600 hover:bg-blue-700 rounded-lg"
          >
            {saving ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Menyimpan...
              </>
            ) : (
              "Simpan"
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
